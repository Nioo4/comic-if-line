...
完整的真实验收证据见 [ACCEPTANCE.md](./ACCEPTANCE.md)。匿名临时 Vercel 部署为 `BLOCKED_PLAN`（匿名计划限制 1–60 秒，应用 Route 固定 `maxDuration=300`）；正式 Vercel 部署为 `BLOCKED_AUTH/NOT_DONE`（当前未登录）。已发布的核心路径视频只覆盖真实 analyze 与上下文确认，不代表完整端到端或稳定性验收。
...
- 正式公共 Vercel URL：`BLOCKED_AUTH/NOT_DONE`，用户授权后才执行 link → 服务端敏感环境变量 → production deploy → 公网 GET/analyze；授权链接不写入文档
