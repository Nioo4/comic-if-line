...
完整的真实验收证据见 [ACCEPTANCE.md](./ACCEPTANCE.md)。当前正式/临时 Vercel 部署标为 `BLOCKED_PLAN/NOT_DONE`（匿名远端 builder 不接受 `maxDuration=300`）；已发布的核心路径视频只覆盖真实 analyze 与上下文确认，不代表完整端到端或稳定性验收。

## 交付链接

- GitHub 仓库 URL：[https://github.com/Nioo4/yinanping-if](https://github.com/Nioo4/yinanping-if)（public 仓库与首个提交已验证可见）
- 公共 Vercel URL：`BLOCKED_PLAN/NOT_DONE`，待具备相应部署计划/授权后使用托管平台环境变量部署
